...
class FileBackedBarrier:
    ...
    def wait_barrier(
        ...
        :param timeout: If provided, the timeout for the wait operation.
        ...
        while True:
            ...
            logger.info("The next wait time is %f. Getting completed activity ids...", next_wait_time)
            completed_activity_ids = self._get_completed_activity_ids()

            logger.info("There are %d completed activities.", len(completed_activity_ids))

            if expected_activity_ids == completed_activity_ids:
                logger.info("All activities completed! Ending wait.")
                return

            total_wait_time = time.time() - start_wait_time
            logger.info("All tasks not finished. Total elapsed wait time is %f.", total_wait_time)
