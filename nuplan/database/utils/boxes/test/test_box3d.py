...
class TestBox3DEncoding(unittest.TestCase):
    ...
    def test_all(self) -> None:
        ...
        box = Box3D((1, 2, 3), (1, 2, 3), Quaternion(0, 0, 0, 0), label=1, score=1.2, velocity=(1, 2, 3),
                    angular_velocity=1, payload=dict({'abc': 'def'}))
        self.assertEqual(box, Box3D.deserialize(box.serialize()))
...
class TestBox3D(unittest.TestCase):
    ...
    def test_rotate(self) -> None:
        ...
        box = Box3D((0.0, 0.0, 0.0), (2.0, 2.0, 2.0), Quaternion(axis=(0., 0., 1.), angle=0))
        theta = np.pi / 2
        box.rotate(Quaternion(axis=(0., 0., 1.), angle=theta))

        assert_array_almost_equal(box.bottom_corners[:, 0], np.array([1., 1., -1.]))  # type: ignore
        assert_array_almost_equal(box.bottom_corners[:, 1], np.array([-1., 1., -1.]))  # type: ignore
        assert_array_almost_equal(box.bottom_corners[:, 2], np.array([-1., -1., -1.]))  # type: ignore
        assert_array_almost_equal(box.bottom_corners[:, 3], np.array([1., -1., -1.]))  # type: ignore

    def test_box_in_image(self) -> None:

        # Box3D at center of image.
        box = Box3D((150.0, 150.0, 150.0), (2.0, 2.0, 2.0), Quaternion(axis=(0., 0., 1.), angle=0))
        intrinsic = np.eye(3)
        imsize = (300, 300)
        ...
        box = Box3D((0.0, 0.0, 0.0), (0.01, 0.01, 0.05), Quaternion(axis=(0., 0., 1.), angle=0))
        ...
        self.assertEqual(box_in_img, True)
        box = Box3D((-10.0, -90.0, -100.0), (2.0, 2.0, 2.0), Quaternion(axis=(10., 20., 1.4), angle=20))
        box_in_img = box_in_image(box, intrinsic, imsize, vis_level=BoxVisibility.NONE)
        self.assertEqual(box_in_img, True)

        # BoxVisibility = ANY is respected
        box = Box3D((0.0, 0.0, 3.0), (2.0, 2.0, 2.0), Quaternion(axis=(0., 0., 1.), angle=0))
        box_in_img = box_in_image(box, intrinsic, imsize, vis_level=BoxVisibility.ANY)
        self.assertEqual(box_in_img, True)
        box = Box3D((-2.0, -2.0, -2.0), (1.0, 1.0, 1.0), Quaternion(axis=(0., 0., 1.), angle=0))
        box_in_img = box_in_image(box, intrinsic, imsize, vis_level=BoxVisibility.ANY)
        self.assertEqual(box_in_img, False)

        # If the depth is more than min_th, it should be visible.
        box = Box3D((10.0, 10.0, 0.51), (1.0, 1.0, 1.0), Quaternion(axis=(0., 0., 1.), angle=0))
        box_in_img = box_in_image(box, intrinsic, imsize, vis_level=BoxVisibility.ANY)
        self.assertEqual(box_in_img, True)
        ...
        box = Box3D((150.0, 150.0, 2.0), (2.0, 2.0, 2.0), Quaternion(axis=(0., 0., 1.), angle=0),
    ...
    def test_translate(self) -> None:
        ...
        box.translate(np.array([12.3, 0., 1.4], dtype=float))
        ...
        box.translate(np.array([-990., 10., -0.4], dtype=float))
        self.assertTrue(np.array_equal(box.center, [-980., 1230., 0.6]))

        # 0 translation
        box = Box3D((10.0, 1220.0, 1.0), (2.0, 2.0, 2.0), Quaternion(axis=(2.2, 0.24, 0), angle=20))
        box.translate(np.array([0., 0., 0.], dtype=float))
    ...
    def test_xflip_no_flip(self) -> None:
        ...
        for input_yaw in (np.pi / 2, -np.pi / 2):
            ...
            box.xflip()
            assert_almost_equal(quaternion_yaw(box.orientation), input_yaw)  # type: ignore
    ...
    def test_xflip_180_flip(self) -> None:
        ...
        for in_yaw, out_yaw in zip(input_yaw, output_yaw):
            ...
            box.xflip()
            assert_almost_equal(quaternion_yaw(box.orientation), out_yaw)  # type: ignore
    ...
    def test_xflip_pos_yaw(self) -> None:
        ...
        for yaw in np.linspace(0, np.pi, 100):
            ...
            box.xflip()
            assert_almost_equal(quaternion_yaw(box.orientation), np.pi - yaw)  # type: ignore
    ...
    def test_yflip_pos_yaw(self) -> None:
        ...
        for yaw in np.linspace(0, np.pi, 100):
            box = Box3D((0, 0, 0), (1, 1, 1), Quaternion(axis=(0, 0, 1), angle=yaw))
            box.yflip()
            assert_almost_equal(quaternion_yaw(box.orientation), -yaw)  # type: ignore
    ...
    def test_velocity_endpoint(self) -> None:
        ...
        box = Box3D((0.0, 0.0, 0.0), (2.0, 2.0, 2.0),
    ...
    def test_front_corners(self) -> None:
        ...
        assert_array_almost_equal(box.front_corners[:, 0], np.array([1, 1, 1]))  # type: ignore
        assert_array_almost_equal(box.front_corners[:, 1], np.array([1, -1, 1]))  # type: ignore
        assert_array_almost_equal(box.front_corners[:, 2], np.array([1, -1, -1]))  # type: ignore
        assert_array_almost_equal(box.front_corners[:, 3], np.array([1, 1, -1]))  # type: ignore
    ...
    def test_rear_corners(self) -> None:
        ...
        assert_array_almost_equal(box.rear_corners[:, 0], np.array([-1, 1, 1]))  # type: ignore
        assert_array_almost_equal(box.rear_corners[:, 1], np.array([-1, -1, 1]))  # type: ignore
        assert_array_almost_equal(box.rear_corners[:, 2], np.array([-1, -1, -1]))  # type: ignore
        assert_array_almost_equal(box.rear_corners[:, 3], np.array([-1, 1, -1]))  # type: ignore
