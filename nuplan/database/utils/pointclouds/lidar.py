...
class LidarPointCloud:
    ...
    def load_pcd_bin(pcd_bin: Union[str, IO[Any], ByteString], pcd_bin_version: int = 1) -> npt.NDArray[np.float32]:
        ...
        if pcd_bin_version == 1:
            ...
            assert False, 'Unknown pcd bin file version: %d' % pcd_bin_version

        return points.T  # type: ignore
    ...
    def from_file(cls, file_name: str) -> LidarPointCloud:
        ...
        if file_name.endswith('.bin'):
            ...
            points = np.load(file_name)  # type: ignore
        else:
    ...
    def remove_close(self, min_dist: float) -> None:
        ...
        dist_from_orig = np.linalg.norm(self.points[:2, :], axis=0)  # type: ignore
    ...
    def range_filter(self,
                     ...
                     zrange: Tuple[float, float] = (-np.inf, np.inf)) -> None:
        """
    ...
    def rotate(self, quaternion: Quaternion) -> None:
        ...
        """
        self.points[:3] = np.dot(quaternion.rotation_matrix.astype(np.float32), self.points[:3])  # type: ignore
    ...
    def render_image(self,
        ...
        points = np.int16(np.round(points[:2, :]))  # type: ignore
        color_values = np.int16(np.round(color_values))  # type: ignore
    ...
    def render_label(self,
        ...
        if id2color is None:
            ...
            for key, color in id2color.items():
                ...
                colors[key] = np.array(color) / 255.
        # Transparent if not in the id list.
        color_list = list(map(lambda x: colors.get(x, np.array((1., 1., 1., 0.))), label))
