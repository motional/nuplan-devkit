...
class TestTransformMatrix(unittest.TestCase):
    ...
    def test_transform_matrix(self) -> None:
        ...
        zero_rotation = Quaternion(axis=(0., 0., 1.), angle=0.0)
        ...
        for _ in range(100):
            ...
            assert_array_almost_equal(tm, tm_test)  # type: ignore
        ...
        for axis_idx, axis in enumerate([x_axis, y_axis, z_axis]):
            for theta in np.linspace(-4.0 * np.pi, 4.0 * np.pi, 100):
                ...
                assert_array_almost_equal(inverse_tm, np.linalg.inv(tm))  # type: ignore
        ...
        for _ in range(100):
            ...
            assert_array_almost_equal(tm1 * tm2, tm2 * tm1)  # type: ignore
...
class TestViewPoints(unittest.TestCase):
    ...
    def test_view_points(self) -> None:
        ...
        for _ in range(100):
            ...
            assert_array_almost_equal(pc1_in_img, pc2_in_img)  # type: ignore
        ...
        for _ in range(100):
            ...
            assert_array_almost_equal(pc3_in_img, pc4_in_img)  # type: ignore
...
class TestMinimumBoundingRectangle(unittest.TestCase):
    ...
    def check_minimum_bounding_rectangle(self,
                                         rect_points: npt.NDArray[np.float64],
                                         points_to_check: List[List[int]]) -> None:
        """
    ...
    def test_lots_of_random_points_in_a_square(self) -> None:
        ...
        points = np.concatenate([points, pts_inside_square])  # type: ignore
    ...
    def test_lots_of_random_points_in_a_rotated_square(self) -> None:
        ...
        rect_points = minimum_bounding_rectangle(np.dot(rot_mat, points.T).T)  # type: ignore

        self.check_minimum_bounding_rectangle(
            rect_points, np.dot(rot_mat, np.array([[0, 0], [1, 0], [1, 1], [0, 1]]).T).T)  # type: ignore
