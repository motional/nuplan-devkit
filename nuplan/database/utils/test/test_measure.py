...
class TestAngleDiffNumpy(unittest.TestCase):
    ...
    def test_zero_shape(self) -> None:
        ...
        y = np.zeros((0,))
        answer = np.zeros((0,))
        assert_almost_equal(measure.angle_diff_numpy(x, y, period), answer)  # type: ignore
...
class TestBirdviewCenterDistanceBox(unittest.TestCase):
    ...
    def test_birdview_center_distance(self) -> None:
        ...
        dist = measure.birdview_center_distance((0., 0., 1., 1., 0.), (0., 0., 1., 1., 0.))
        self.assertEqual(dist, 0)
        dist = measure.birdview_center_distance((0., 0., 1., 1., 0.), (1., 0., 1., 1., 0.))
        self.assertEqual(dist, 1)
        dist = measure.birdview_center_distance((0., 0., 1., 1., 0.), (1., 1., 1., 1., 0.))
    ...
    def test_birdview_center_distance_box(self) -> None:
        ...
        dist = measure.birdview_center_distance_box(Box3D((0, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)),
                                                    Box3D((0, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)))
        self.assertEqual(dist, 0)
        dist = measure.birdview_center_distance_box(Box3D((0, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)),
                                                    Box3D((1, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)))
        self.assertEqual(dist, 1)
        dist = measure.birdview_center_distance_box(Box3D((0, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)),
                                                    Box3D((1, 1, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)))
        self.assertAlmostEqual(dist, 1.4142135623730951)

        # compare complicated for test_birdview_center_distance_box and birdview_center_distance
        dist1 = measure.birdview_center_distance_box(Box3D((4, 5, 0), (2, 2, 1), Quaternion(0, 0, 0, 0)),
                                                     Box3D((1, 4, 0), (2, 4, 1), Quaternion(axis=(0, 0, 1),
                                                                                            angle=np.pi / 3)))
        dist2 = measure.birdview_center_distance((4., 5., 2., 2., 0.), (1., 4., 2., 4., np.pi / 3.))
...
class TestHausdorffDistance(unittest.TestCase):
    ...
    def test_hausdorff_distance(self) -> None:
        ...
        dist = measure.hausdorff_distance((0., 0., 1., 1., 0.), (0., 0., 1., 1., 0.))
        self.assertEqual(dist, 0)

        dist = measure.hausdorff_distance((0., 0., 1., 1., 0.), (1., 0., 1., 1., 0.))
        self.assertEqual(dist, 1.0)

        dist = measure.hausdorff_distance((0., 0., 1., 1., 0.), (1., 1., 1., 1., 0.))
        self.assertAlmostEqual(dist, 1.4142135623730951)

        # 2 boxes of same center and width=1, length=2 but one is rotated.
        dist = measure.hausdorff_distance((1., 1., 1., 2., 0.), (1., 1., 1., 2., np.pi / 2.))
        self.assertAlmostEqual(dist, 0.5)

        # 2 boxes of same size, one is overlapping by w/2.
        dist = measure.hausdorff_distance((1., 1., 1., 2., 0.), (1., 1.5, 1., 2., 0.))
        self.assertAlmostEqual(dist, 0.5)

        # box1 is inside box2. box2 is double the size of box1.box1 and box2 is aligned in x axis.
        dist = measure.hausdorff_distance((1., 1., 1., 2., 0.), (1., 2., 2., 4., 0.))
        ...
        dist = measure.hausdorff_distance((0., 0., 2. / np.sqrt(2.), 2. / np.sqrt(2.), 0.),
                                          (0., 1. / np.sqrt(2.), 1., 1., np.pi / 4.))
    ...
    def test_hausdorff_distance_box(self) -> None:
        ...
        dist = measure.hausdorff_distance_box(Box3D((0, 0, 0), (1, 1, 1), Quaternion(0, 0, 0, 0)),
                                              Box3D((0, 0, 0), (1, 1, 10), Quaternion(0, 0, 0, 0)))
        self.assertEqual(dist, 0)
        ...
        dist1 = measure.hausdorff_distance_box(Box3D((4, 5, 0), (2, 2, 1), Quaternion(0, 0, 0, 0)),
                                               Box3D((1, 4, 0), (2, 4, 1), Quaternion(axis=(0, 0, 1), angle=np.pi / 3)))
        dist2 = measure.hausdorff_distance((4., 5., 2., 2., 0.), (1., 4., 2., 4., np.pi / 3.))
        self.assertEqual(dist1, dist2)
...
class TestPseudoIOU(unittest.TestCase):
    ...
    def test_pseudo_distance_2pi(self) -> None:
        ...
        a = (-100., -100., 100., 100., 0.)
        b = (0., 0., 1., 1., math.pi / 2.)
        self.assertAlmostEqual(measure.birdview_corner_angle_mean_distance(
            a, b, period=period), (398 + math.pi / 2) / 5)
    ...
    def test_pseudo_distance_box_pi(self) -> None:
        ...
        self.assertAlmostEqual(measure.birdview_corner_angle_mean_distance_box(a, b, period=period),
                               (398 + math.pi / 2) / 5)
    ...
    def test_pseudo_distance_box_2pi(self) -> None:
        ...
        a = Box3D(center=(-100, -100, 0), size=(100, 100, 1), orientation=Quaternion(axis=[0, 0, 1], angle=0))
        b = Box3D(center=(0, 0, 0), size=(1, 1, 1), orientation=Quaternion(axis=[0, 0, 1], angle=math.pi / 2))
        self.assertAlmostEqual(measure.birdview_corner_angle_mean_distance_box(
            a, b, period=period), (398 + math.pi / 2) / 5)
...
class TestIOU(unittest.TestCase):
    ...
    def test_intersection(self) -> None:
        ...
        b = (0., 0., 100., 100.)
        self.assertEqual(measure.intersection(a, b), 10000.)

        # no intersection with box of no size
        b = (100., 100., 100., 100.)
        self.assertEqual(measure.intersection(a, b), 0.)
        ...
        # Area = 50 * 50 = 2500
        b = (50., 50., 150., 150.)
        self.assertEqual(measure.intersection(a, b), 2500.)
    ...
    def test_union(self) -> None:
        ...
        b = (0., 0., 100., 100.)
        self.assertEqual(measure.union(a, b), 10000.)
        ...
        b = (100., 100., 200., 200.)
        self.assertEqual(measure.union(a, b), 20000.)
        ...
        b = (50., 50., 150., 150.)
        self.assertEqual(measure.union(a, b), 17500.)
...
class TestLongLatDecomp(unittest.TestCase):
    ...
    def test_euclidean(self) -> None:
        ...
        for _ in range(5):
            ...
            dist1 = np.linalg.norm([long, lat])  # type: ignore
            dist2 = np.linalg.norm(gt - est)  # type: ignore
