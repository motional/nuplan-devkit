...
class PlannerExpertFinalHeadingErrorStatistics(MetricBase):
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        :return the estimated metric.
        """
        final_heading_errors = self._planner_expert_average_l2_error_metric.final_heading_errors
        ego_timestamps_sampled = self._planner_expert_average_l2_error_metric.ego_timestamps_sampled

        time_series = TimeSeries(
            unit='radian',
            time_stamps=ego_timestamps_sampled[: len(final_heading_errors)],
            values=final_heading_errors,
        )
