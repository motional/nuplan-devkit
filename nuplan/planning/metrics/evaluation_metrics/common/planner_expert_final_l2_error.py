...
class PlannerExpertFinalL2ErrorStatistics(MetricBase):
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        :return the estimated metric.
        """
        final_displacement_errors = self._planner_expert_average_l2_error_metric.final_displacement_errors
        ego_timestamps_sampled = self._planner_expert_average_l2_error_metric.ego_timestamps_sampled

        time_series = TimeSeries(
            unit='meters',
            time_stamps=ego_timestamps_sampled[: len(final_displacement_errors)],
            values=final_displacement_errors,
        )
