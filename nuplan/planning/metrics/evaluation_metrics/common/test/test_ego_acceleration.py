...
class TestEgoAcceleration(unittest.TestCase):
    """ Run ego_acceleration unit tests. """

    def setUp(self) -> None:

        self.metric_name = "ego_acceleration"
        self.ego_acceleration_metric = EgoAccelerationStatistics(name=self.metric_name, category='Dynamics')
    ...
    def test_metric_name(self) -> None:
        ...
        self.assertEqual(self.ego_acceleration_metric.name, self.metric_name)

    def test_compute(self) -> None:
        """ Test compute. """

        results = self.ego_acceleration_metric.compute(self.history)

        self.assertTrue(isinstance(results, list))
        metric = results[0]
        statistics = metric.statistics
        self.assertEqual(statistics[MetricStatisticsType.MAX].value, 1)
        ...
        self.assertEqual(time_series.values, [0, 1.0, 0.5])
