...
class TestEgoLatAcceleration(unittest.TestCase):
    """ Run ego_lat_acceleration unit tests. """

    def setUp(self) -> None:

        self.metric_name = "ego_lat_acceleration"
        self.ego_lat_acceleration_metric = EgoLatAccelerationStatistics(name=self.metric_name, category='Dynamics')
    ...
    def setup_history(self) -> SimulationHistory:
        ...
        ego_states = [
            ...
            EgoState.from_raw_params(StateSE2(664431.1930625531, 3997651.6249544094, np.pi / 2),
                                     ...
                                     acceleration_2d=StateVector2D(x=1.0, y=2.0),
                                     tire_steering_angle=0,
            ...
            EgoState.from_raw_params(StateSE2(664432.1930625531, 3997652.6249544094, np.pi / 4),
                                     ...
                                     acceleration_2d=StateVector2D(x=0.5, y=-3.0),
                                     tire_steering_angle=0,
    ...
    def test_metric_name(self) -> None:
        ...
        self.assertEqual(self.ego_lat_acceleration_metric.name, self.metric_name)

    def test_compute(self) -> None:
        """ Test compute. """

        results = self.ego_lat_acceleration_metric.compute(self.history)

        self.assertTrue(isinstance(results, list))
        metric = results[0]
        statistics = metric.statistics
        self.assertEqual(statistics[MetricStatisticsType.MAX].value, 2.0)
        self.assertEqual(statistics[MetricStatisticsType.MIN].value, -3.0)
        self.assertEqual(statistics[MetricStatisticsType.P90].value, 2.8)

        time_series = metric.time_series
        assert isinstance(time_series, TimeSeries)
        self.assertEqual(time_series.time_stamps, [0, 1, 2])
        self.assertEqual(time_series.values, [1.0, 2.0, -3.0])
