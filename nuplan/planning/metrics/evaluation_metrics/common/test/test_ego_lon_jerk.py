...
class TestEgoLonJerk(unittest.TestCase):
    """ Run ego_lon_jerk unit tests. """

    def setUp(self) -> None:

        self.metric_name = "ego_lon_jerk"
        self.ego_lon_jerk = EgoLonJerkStatistics(name=self.metric_name, category='Dynamics')
    ...
    def test_metric_name(self) -> None:
        ...
        self.assertEqual(self.ego_lon_jerk.name, self.metric_name)

    def test_compute(self) -> None:
        """ Test compute. """

        results = self.ego_lon_jerk.compute(self.history)

        self.assertTrue(isinstance(results, list))
        metric = results[0]
        statistics = metric.statistics
        self.assertEqual(np.round(statistics[MetricStatisticsType.MAX].value, 2), 1.36)  # type:ignore
        self.assertEqual(np.round(statistics[MetricStatisticsType.MIN].value, 2), -0.36)  # type:ignore
        self.assertEqual(np.round(statistics[MetricStatisticsType.P90].value, 2), 1.19)  # type:ignore
        ...
        self.assertEqual(np.round(time_series.values, 2).tolist(), [-0.36, 0.07, 0.5, 0.93, 1.36])  # type:ignore
