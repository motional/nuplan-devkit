...
from nuplan.planning.simulation.observation.idm.utils import get_closest_agent_in_position, is_agent_behind


class TimeGapToRearAgent(MetricBase):
    """Time Gap to rear agents metric."""

    def __init__(self, name: str, category: str) -> None:
        """Initializes the TimeGapToRearAgent class
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        for sample in history.data:
            ...
            if closest_agent is not None:
                agent_velocity = np.hypot(closest_agent.velocity.x, closest_agent.velocity.y)
            else:
                agent_velocity = 0

            # clamp to avoid zero division
            time_gap_value = closest_distance / (max(agent_velocity, 0.1))
