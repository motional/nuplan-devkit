...
class TestOverviewTab(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.overview_tab = OverviewTab(file_paths=[self.nuboard_file],
                                        metric_categories=['Dynamic'],
                                        doc=self.doc)
    ...
    def test_update_overview(self) -> None:
        ...
        self.overview_tab._scenario_type_multi_choice.value = ['Test']

        # Update scenario name choices
        self.overview_tab._metric_name_multi_choice.value = ['ego_acceleration_statistics']

        self.assertEqual(len(self.overview_tab._tables), 1)
        self.assertTrue('Dynamic' in self.overview_tab._tables)
        self.assertEqual(len(self.overview_tab._plot_layout.children), 2)

    def test_file_paths_on_change(self) -> None:
        """ Test file_paths_on_change function. """

        self.overview_tab.file_paths_on_change(file_paths=[])
        self.assertEqual(self.overview_tab._scenario_type_multi_choice.value, [])
        self.assertEqual(self.overview_tab._scenario_type_multi_choice.options, [])
        self.assertEqual(self.overview_tab._metric_name_multi_choice.value, [])
        self.assertEqual(self.overview_tab._metric_name_multi_choice.options, [])

    def test_panel(self) -> None:
        """ Test panel properties. """

        self.assertIsInstance(self.overview_tab.panel, Panel)
        self.assertIsInstance(self.overview_tab.panel.child, LayoutDOM)
