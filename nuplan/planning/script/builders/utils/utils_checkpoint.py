...
def find_last_checkpoint_in_dir(group_dir: pathlib.Path, experiment_time: pathlib.Path) \
...
def extract_last_checkpoint_from_experiment(output_dir: pathlib.Path, date_format: str) -> Optional[pathlib.Path]:
    ...
    """
    date_times = [datetime.strptime(dir.name, date_format) for dir in output_dir.parent.iterdir() if
                  dir != output_dir]
