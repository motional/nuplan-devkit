...
class ValidateSetupCallback(pl.Callback):
    ...
    def on_before_accelerator_backend_setup(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        # Make sure this is run for the correct setup
        assert isinstance(pl_module, PlanningModule), "pl_module is not PlanningModule!"
        ...
        assert len(model_out_feature) == len(
            datamodule_target_types), "Length of model output feature and builder features is not the same!"
